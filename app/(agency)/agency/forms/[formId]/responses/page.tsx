...
interface FieldInfo { id: string; type: string; label: string }
...
export default function AgencyFormResponsesPage() {
  ...
  const buildColumns = (fields: FieldInfo[]): DisplayColumn[] => {
    ...
    fields.forEach((f) => {
      ...
      if (f.type === 'ZIP') {
        ...
        cols.push({ key: f.id, label: f.label, getValue: (d) => {
          const val = d[f.id] as { name?: string; size?: number } | null
          if (!val || typeof val !== 'object') return ''
          const size = val.size
            ? val.size < 1024 * 1024 ? `${(val.size/1024).toFixed(1)}KB` : `${(val.size/(1024*1024)).toFixed(1)}MB`
            : ''
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!res.ok) { router.push('/agency/forms'); return }
  ...
  const handleOpenDetail = async (r: ResponseItem) => {
    ...
    if (!r.isRead) await handleToggleRead(r)
  }
  ...
  const handleExportCSV = useCallback(() => {
    ...
    const rows = responses.map((r) => [new Date(r.createdAt).toLocaleString('ja-JP'), ...columns.map((c) => c.getValue(r.data))])
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <Button variant="ghost" size="sm" className="-ml-2 mb-1" onClick={() => router.push('/agency/forms')}>
            ← フォーム一覧へ
          </Button>
      ...
      <div className="flex items-center gap-3 mb-4">
        <Input
          ...
          className="max-w-sm"
        />
        <Button variant={filterUnread ? 'default' : 'outline'} size="sm" onClick={() => setFilterUnread((v) => !v)}>
          未読のみ
        </Button>
      </div>
              ...
              <tbody>
                {filteredResponses.map((r) => (
                  ...
                  >
                    ...
                    {columns.slice(0, 6).map((c) => (
                      <td key={c.key} className="p-3 max-w-[200px] truncate">{c.getValue(r.data)}</td>
                    ))}
                    {columns.length > 6 && <td className="p-3">...</td>}
                    <td className="p-3" onClick={(e) => e.stopPropagation()}>
                      <Button size="sm" variant="ghost" className="text-[hsl(var(--destructive))]" onClick={() => setDeleteTarget(r)}>削除</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <Dialog open={!!selectedResponse} onOpenChange={() => setSelectedResponse(null)}>
        <DialogHeader><DialogTitle>送信データ詳細</DialogTitle></DialogHeader>
        {selectedResponse && (
          <>
            <div className="mt-4 space-y-3 max-h-[60vh] overflow-y-auto">
              <div className="flex items-center justify-between text-sm text-[hsl(var(--muted-foreground))]">
                <span>送信日時: {new Date(selectedResponse.createdAt).toLocaleString('ja-JP')}</span>
                <Button size="sm" variant="ghost" onClick={() => handleToggleRead(selectedResponse)}>
              ...
              {columns.map((c) => {
                ...
                const fileVal = isFile ? rawVal as { url: string; name: string } : null
                return (
                  <div key={c.key} className="flex border-b border-[hsl(var(--border))] pb-2">
                    <span className="w-1/3 text-sm font-medium text-[hsl(var(--muted-foreground))]">{c.label}</span>
                    <span className="w-2/3 text-sm break-all">
                      {fileVal ? (
                        <a href={fileVal.url} target="_blank" rel="noopener noreferrer" className="text-[hsl(var(--primary))] underline">
            ...
            <DialogFooter>
              <Button variant="destructive" size="sm" onClick={() => { setDeleteTarget(selectedResponse); setSelectedResponse(null) }}>削除</Button>
