...
interface FormInfo { id: string; title: string }
interface FieldInfo { id: string; type: string; label: string }
interface ResponseItem {
  id: string
  formId: string
  form: { title: string; steps: { fields: FieldInfo[] }[] }
...
function ResponsesContent() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) { router.push('/agency'); return }
                ...
                {filtered.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3 max-w-[200px] truncate text-[hsl(var(--muted-foreground))]">{r.memo || ''}</td>
                      <td className="p-3" onClick={(e) => e.stopPropagation()}>
                        <Button size="sm" variant="ghost" className="text-[hsl(var(--destructive))]" onClick={() => setDeleteTarget(r)}>
                          削除
                        </Button>
                      </td>
                    </tr>
          ...
          <>
            <div className="mt-4 space-y-4 max-h-[60vh] overflow-y-auto">
              <div className="flex items-center justify-between text-sm text-[hsl(var(--muted-foreground))]">
                ...
                <a href={`/agency/forms/${selectedResponse.formId}/responses`} className="text-[hsl(var(--primary))] underline text-xs">
                  フォーム別に見る
                </a>
              </div>
              <div className="space-y-2">
                {(() => {
                  ...
                  selectedResponse.form.steps?.forEach((s) => s.fields?.forEach((f) => { fieldMap[f.id] = f }))
                  ...
                  return displayEntries.map(([key, val]) => {
                    ...
                    if (typeof val === 'object' && !Array.isArray(val) && val !== null && 'url' in val) {
                    ...
                    else if (typeof val === 'object' && !Array.isArray(val)) display = Object.values(val as object).filter(Boolean).join(' ')
                    else if (Array.isArray(val)) display = val.join(', ')
                    else if (typeof val === 'boolean') display = val ? '同意する' : '同意しない'
...
export default function AgencyResponsesPage() {
  return (
